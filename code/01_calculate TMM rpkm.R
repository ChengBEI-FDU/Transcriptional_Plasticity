...
calculating_TMM_RPKM_within_batch <- function(counts_path, # path of htseq count files (.count)
                                              sample_study, # data frame. Column "Run" is SRA accession (refer to htseq-count file name) and column "BioProject" is bio-projects
                                              gene_length, # data frame. Column "gene" is locus tag and column "length" is gene length
                                              filter_lib_size = 1000000, # a numeric that sample with library size <= 'filter_lib_size' will be excluded
                                              filter_geneLength = 150, # genes shorter than 150 bp will be removed
  ...
  # calculate TMM normalized RPKM (Bio Project-specific TMM normalization)
  RPKM.0 <- data.frame(row.names = count_gene, V1 = rep(NA, length(count_gene)))
  for (i in 1:length(sample_project)) {
    temp.sample <- sample_study$Run[sample_study$BioProject == sample_project[i]] 
    temp.count <- raw.counts[,colnames(raw.counts) %in% temp.sample]
...
#### Input data ####
# sample
sample_Mtb <- read.xlsx("./data/original data/RNA seq data/sample/Mtb_SRA.xlsx")
sample_Mab <- read.xlsx("./data/original data/RNA seq data/sample/Mabsc_SRA.xlsx")
sample_Msmeg <- read.xlsx("./data/original data/RNA seq data/sample/Msmeg_SRA.xlsx")
...
# gene length
gtf_Mtb <- as.data.frame(import("./data/original data/genome ref/Mtb_GCF_000195955.2_ASM19595v2_genomic.gff"))
gtf_Mab <- as.data.frame(import("./data/original data/genome ref/Mycobacterium_abscessus_ATCC_19977_gff_v4.gff"))
gtf_Msmeg <- as.data.frame(import("./data/original data/genome ref/Mycobacterium_smegmatis_MC2-155_gff_v4.gff"))
...
rpkm_Mtb <- calculating_TMM_RPKM_allSamples(counts_path = "./data/original data/RNA seq data/htseqcount_new/Mtb/", 
                                            sample_study = sample_Mtb, gene_length = geneLength_Mtb,
                                            filter_gene = filter_gene_mtb)
setwd("/Users/beicheng/Desktop/MTB Transcriptome the Big Data/results9/article/Revision_Version_0127/Data and code/")
write.csv(rpkm_Mtb, "./data/tmp data/unfiltered tmm rpkm Mtb.csv")


# Msm
rpkm_Msmeg <- calculating_TMM_RPKM_allSamples(counts_path = "./data/original data/RNA seq data/htseqcount_new/Msmeg/", 
                                              sample_study = sample_Msmeg, gene_length = geneLength_Msmeg)
setwd("/Users/beicheng/Desktop/MTB Transcriptome the Big Data/results9/article/Revision_Version_0127/Data and code/")
write.csv(rpkm_Msmeg, "./data/tmp data/unfiltered tmm rpkm Msm.csv")


# Mab
pla.gene_Mab <- gtf_Mab$Locus[gtf_Mab$seqnames %in% "NC_010394.1"]
rpkm_Mab <- calculating_TMM_RPKM_allSamples(counts_path = "./data/original data/RNA seq data/htseqcount_new/Mab/", 
                                            sample_study = sample_Mab, gene_length = geneLength_Mab,
                                            filter_gene = pla.gene_Mab) # remove plasmid genes
setwd("/Users/beicheng/Desktop/MTB Transcriptome the Big Data/results9/article/Revision_Version_0127/Data and code/")
write.csv(rpkm_Mab, "./data/tmp data/unfiltered tmm rpkm Mab.csv")


#### Calculate within-batch TMM normalized RPKM ####
# Mtb
filter_gene_mtb <- gtf_Mtb$locus_tag[gtf_Mtb$gene %in% c("rrl","rrs","rrf","rnpB")] # remove ribosome RNA
filter_gene_mtb <- unique(c(filter_gene_mtb,geneLength_Mtb$gene[grep(pattern="nc", geneLength_Mtb$gene)])) # remove noncoding RNA

rpkm_withinbatch <- calculating_TMM_RPKM_within_batch(counts_path = "./data/original data/RNA seq data/htseqcount_new/Mtb/", 
                                              sample_study = sample_Mtb, gene_length = geneLength_Mtb,
                                              filter_gene = filter_gene_mtb)
setwd("/Users/beicheng/Desktop/MTB Transcriptome the Big Data/results9/article/Revision_Version_0127/Data and code/")
write.csv(rpkm_withinbatch, "./data/tmp data/unfiltered within-batch tmm rpkm Mtb.csv")
